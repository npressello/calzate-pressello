    ...
    <>
        <nav className={`${props.mobileOpen} md:flex md:ml-auto mt-6 md:mt-0`} id='navbar-collapse'>
          <ul className="flex flex-col md:flex-row md:justify-between">
            <li className='mx-2'><NavLink onClick={props.toggleSelf()} exact="true" to={`/category/${0}`} className={({isActive}) => "block md:inline rounded md:rounded-none mx-2 px-2 py-1 bg-calzate-400 md:bg-white border-calzate-400 text-calzate-900 md:text-calzate-100 " + (isActive ? 'font-medium md:border-b-4' : 'md:border-b hover:md:border-b-4')}>Inicio</NavLink></li>
            {cat.map( (c, index) => <li key={index} className='mx-2'><NavLink onClick={props.toggleSelf()} exact="true" to={`/category/${c.id}`} className={({isActive}) =>  "block md:inline rounded md:rounded-none mx-2 px-2 py-1 hover:bg-calzate-700 md:hover:bg-white border-calzate-400 text-calzate-100 " + (isActive ? 'font-medium md:border-b-4' : 'md:border-b transition-all hover:md:border-b-4')}>{c.name}</NavLink></li>)}
